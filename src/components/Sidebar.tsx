 ...
 */
function ConsoleButton({ collapsed }: { collapsed: boolean }) {
  const openWindow = async () => {
    try {
      await (window as any).streamControl?.twitchWebhook?.openConsole?.();
    } catch {
      // If the window fails to open, ignore — desktop environment may vary.
    }
  };
  ...
  return (
    <div className="mt-2 px-3">
      <button
        onClick={openWindow}
        className="w-full flex items-center justify-center gap-1.5 rounded-md bg-[#9146FF]/10 text-[#9146FF] hover:bg-[#9146FF]/20 text-[10px] font-bold py-1.5 transition-colors"
        title="Open console window"
      >
        <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="18" height="14" rx="2" strokeLinecap="round" strokeLinejoin="round" /><line x1="8" y1="9" x2="16" y2="9" strokeLinecap="round" /></svg>
    ...
    >
      ...
      {!["bridge", "webhooks", "settings"].includes(activeId) && (
        <div className="px-3 pt-3 pb-3 border-t border-white/10">
          ...
          {/* Session ID button for headless / no-console access */}
          <ConsoleButton collapsed={collapsed} />
        </div>
      )}
